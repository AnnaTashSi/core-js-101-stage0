...
function findFirstSingleChar(str) {
  ...
  const result = chars.find((character) => charCount[character] === 1);
...
function isBracketsBalanced(str) {
  ...
  const isUnbalanced = str.split('').some((char) => {
    if (brackets[char]) {
      ...
      if (brackets[lastOpen] !== char) return true;
    }
    return false;
  });
...
function toNaryString(num, n) {
  if (n < 2 || n > 10) {
    throw new Error('Radix n must be between 2 and 10.');
  }
  if (num === 0) return '0';
...
function getCommonDirectoryPath(pathes) {
  ...
  if (commonSegments.length === 0) {
    const allAbsolute = pathes.every((p) => p.startsWith('/'));
    if (allAbsolute) {
      return '/';
    }
    return '';
...
function getMatrixProduct(m1, m2) {
  ...
  if (colsM1 !== rowsM2) {
    throw new Error('Невозможно умножить матрицы: несовместимые размеры.');
  }
...
function evaluateTicTacToePosition(position) {
  ...
  // Проверка победителя с помощью методов массива
  const winnerX = lines.some((line) => line.every((cell) => cell === 'X'))
    ? 'X'
    : undefined;
  const winner0 = lines.some((line) => line.every((cell) => cell === '0'))
    ? '0'
    : undefined;
